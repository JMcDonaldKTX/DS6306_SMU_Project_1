...
### With Budweiser being a Lager, we wanted to see how Budweiser stacks up against other craft beer lagers and offer some insight into IPA and American Pale Ales.
...
#ipaDF <- left_join(ipaDF, regions, by = c("State" = "State.Code"))
#regions %>% rename(StateName = State, State = State.Code)
head(regions)
head(ipaDF)
ipaDF <- merge(ipaDF,regions, by.x = 10, by.y = 2, na.rm = True)
head(ipaDF)
```
